...
get.table.for.state = function(s) {
  get_acs(
    geography = 'tract', # change geography
    state = s, # plug in state fips on each get call
...
occ.tables = state.fips %>% 
  mutate(
    # run a function over all items in a column
    # save the object into a "list-column"
    state.data = map(
      state, # the column to loop over
      get.table.for.state # the function to run
    )
  )

# this has nested data frames for each of the states
occ.tables
...
calcs %>% arrange(-zval) %>% write_csv('crowding-tracts.csv')

# occ.table %>% write_csv('occupancy-data-tracts.csv')
occ.table %>% write_rds('occupancy-data-tracts.rds', compress = 'gz')
